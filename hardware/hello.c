...
void print_pixel_axis() {
  ...
  //printf("position: %02d %02d\n",vla.position.x_axis, vla.position.y_axis);
}

/* Set the pixel axis */
void set_pixel_axis(const vga_pixel_axis_t *p){
  vga_pixel_arg_t vla;
  vla.position = *p;
...
void print_background_color() {
  ...
  //printf("color: %02x %02x %02x\n",vla.background.red, vla.background.green, vv
la.background.blue);
}

/* Set the background color */
void set_background_color(const vga_pixel_color_t *c)
{
  vga_pixel_arg_t vla;
  vla.background = *c;
...
int** image_vga() {
    FILE *coe_file = fopen("image_pixel.coe", "r");
    if (!coe_file) {
...
//    fseek(coe_file, 0, SEEK_END);
    ...
    // Get the current position of the file pointer, which is the length of the  
file
...
int main()
{
  ...
  static vga_pixel_axis_t position = {
    {0, 0} /* pixel axis x,y */
  };
...
# define COLORS 9
  ...
  for (int i = 0 ; i < 480 ; i++) {
    for (int j = 0; j < 640; j++) {
        ...
        print_background_color();
        position.x_axis = position.x_axis + 1;
        set_pixel_axis(&position);
        print_pixel_axis();
}
    position.y_axis = position.y_axis + 1;
    position.x_axis = 0;
}
